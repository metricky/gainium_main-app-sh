 ...
 */
class BotHelper extends MainBot<BotSchema> {
  ...
  private prevGrids: Grid[] | null = null
  /** Array to store active grids */
  private grids: Grid[] | null = null
  ...
  private lockTpSlCheck: boolean
  /** Swap order process lock */
  private swapLock = false
  /** Swap order type */
  private swapType: BuyTypeEnum = BuyTypeEnum.proceed
  /** Swap sell orders count */
  private swapSellCount = 0
  /** Swap qty */
  private swapOrderQty = 0
  /** Close order timer */
  ...
  /** Block limit */
  private blockLimit = false
  /** Swap assets started */
  private swapAssetsStarted = false
  /** Profit currency was changed */
  private profitCurrencyChanged = false
  ...
  private startTimeoutTime = 0
  private limitTimer: NodeJS.Timeout | null = null
  private enterMarketTimer: NodeJS.Timeout | null = null
  /**
   * Prepare DB instaces
   *
  ...
  get isShort() {
    return this.futures && this.futuresStrategy !== FuturesStrategyEnum.neutral
        ...
        (acc, grid) => {
          if (
            ...
            return {
              qty:
                acc.qty +
                grid.qty * (this.futures ? 1 : !this.isShort ? 1 : grid.price),
    ...
    if (this.data && this.exchange && fee && ed) {
      if (
        ...
        if (!this.data.initialPrice) {
          const lastPrice = await this.getLatestPrice(pair)
          if (lastPrice !== 0) {
            this.generateInitialBalances(lastPrice, InitialPriceFromEnum.start)
          }
        }
    ...
    if (this.serviceRestart) {
      ...
      if (lastPrice) {
        ...
        for (const o of activeRegularOrders.filter(
          ...
          const exchangeData = await this.getOrder(o.clientOrderId, pair, true)
          if (!exchangeData || !exchangeData.data) {
            ...
            if (exchangeData.status === StatusEnum.notok) {
              ...
              if (updatedOrder.status === 'CANCELED') {
                ...
                this.handleLog(`Order ${updatedOrder.clientOrderId} is FILLED.`)
                this.updateOrderOnDb(updatedOrder)
  ...
  private async placeRegularOrder(order: Grid, ed: ClearPairsSchema) {
    if (!this.isOrderExist(order, TypeOrderEnum.regular)) {
      ...
      const reduce =
        ...
        (order.side === OrderSideEnum.sell && currentSide === PositionSide.LONG)
  ...
  private async processFeeBalance(order: Order) {
    if (
      ...
      this.emit('bot settings update', { feeBalance: this.data.feeBalance })
      if (this.data.feeBalance <= 0 && order.typeOrder !== TypeOrderEnum.fee) {
  ...
  private async processFilledOrder(
    ...
    if (!this.loadingComplete) {
      ...
      this.runAfterLoadingQueue.push(() =>
        this.processFilledOrder.bind(this)(order, updateTime, skipLimitOrders),
      )
      this.endMethod(_id)
  ...
  private async checkPriceToStart() {
    if (!this.data) {
      return true
    }
    const {
  ...
  private clearClassProperties(clearRedis = false, start = false) {
  ...
  async start(): Promise<void> {
    ...
    if (
      ...
      this.handleErrors(`Bot cannot run in hedge mode. Bot will stop`, 'start')
      this.serviceRestart = false
      this.finishLoad = true
      this.endMethod(_id)
      return await this.stop()
  ...
  private async profitAfterPositionClosed(result: Order) {
    if (!this.data || !this.futures || this.data.position.qty === 0) {
      return
    }
    const { qty, price, side } = this.data.position
    ...
    if (this.data) {
      if (!this.futures) {
        ...
        const res = await this.transactionDb.createData({
          ...
          side: order.side,
          amountBaseBuy: order.side === OrderSideEnum.buy ? +order.origQty : 0,
          amountQuoteBuy:
            order.side === OrderSideEnum.buy
              ? +(
                  order.cummulativeQuoteQty ?? +order.price * +order.executedQty
          ...
          idSell: order.side === OrderSideEnum.sell ? order.clientOrderId : '',
  ...
  async stop(
    ...
    if (this.data?.status === BotStatusEnum.closed) {
      ...
      await this.unsubscribeFromLastUsdData(`${this.data.settings.pair}_quote`)
  ...
  async checkBalances(order: Order) {
    ...
    const filteredOrder = orders.data.result
      ...
      .filter((o) =>
        this.data?.exchange === ExchangeEnum.bybit ? +o.executedQty > 0 : true,
      )
    const last = order
    if (!last) {
      return this.processError(
  ...
  async createTransaction(o: Order): Promise<void> {
    ...
    if (this.initialGrid && this.exchange) {
      ...
      const transaction: Omit<ClearTransactionSchema, '_id'> = {
        ...
        cummulativeProfitQuote: this.data.profit.total + profitQuote - comQuote,
            ...
            freeTotalUsd:
              this.data.profit.freeTotalUsd || this.data?.profit?.totalUsd
                ? (this.data.profit.freeTotalUsd ||
                    this.data?.profit?.totalUsd) + (freeProfitUsd || profitUsdt)
   ...
   */
  private async generateInitialBalances(
    price: number,
    ...
    if (!onlyBalances) {
      ...
      if (orders.status === StatusEnum.notok) {
        ...
        const buyQuote = buy.reduce((acc, v) => acc + +v.origQty * +v.price, 0)
    ...
    if (this.data?.settings.newBalance) {
      ...
      const allBuysQuote = allBuys.reduce((acc, v) => acc + v.qty * v.price, 0)
    ...
    if (this.data) {
      if (
        ...
        this.handleLog('Reset real initial balances by initial balance change')
        this.data.realInitialBalances = null
        this.data.lastBalanceChange = null
        this.updateData({
          realInitialBalances: null,
  ...
  private async generateCurrentBalances(
    ...
    const levels = {
      ...
      all: {
        buy:
          currentGrids.filter((o) => o.side === OrderSideEnum.buy).length || 0,
        sell:
          currentGrids.filter((o) => o.side === OrderSideEnum.sell).length || 0,
      },
    }
    const data: Partial<ClearBotSchema> = {
      currentBalances,
  ...
  tpSl(lastPrice: number): { value: TpSlReturn; text: string } {
    if (this.data && (this.data.settings.tpSl || this.data.settings.sl)) {
      ...
      if (
        (tpSlCondition === 'valueChanged' && tpPerc && initialPrice && tpSl) ||
        ...
        if (this.futures) {
          ...
          const totalPerc = valueChange / (initialValue / this.currentLeverage)
          if (
          ...
          if (slCondition === 'valueChanged' && slPerc && initialPrice && sl) {
            if (totalPerc <= slPerc) {
              const value = this.math.round(totalPerc * 100)
  ...
  async sellBaseAmount(): Promise<number> {
    ...
    if (!this.isShort) {
      ...
      return this.math.round(
        Math.min(
          ...
          (this.data?.currentBalances.base || 0) + (this.data.feeBalance ?? 0),
        ),
        await this.baseAssetPrecision(ed.pair),
        true,
      )
    ...
    const currentSellOrders = orders
      .filter(
        (o) => o.status === 'PARTIALLY_FILLED' && o.side === OrderSideEnum.sell,
      )
      .reduce((acc, v) => acc + +v.executedQty, 0)
    const currentOrders = currentSellOrders + currentBuyOrders
    ...
    if (result !== TpSlReturn.none) {
      ...
      if (
        ...
        if (payload.grid && payload.additional) {
          ...
          if (resultOrder && resultOrder.status === 'FILLED' && this.futures) {
          ...
          if (resultOrder && resultOrder.status === 'FILLED' && !this.futures) {
            ...
            const currentBalances = {
              base:
                this.data.currentBalances.base +
                +(resultOrder.executedQty || resultOrder.origQty) * multiplier,
              quote:
                this.data.currentBalances.quote +
            ...
            const res = await this.transactionDb.createData({
              ...
              priceSell:
                resultOrder.side === OrderSideEnum.sell
                  ...
                  : 0,
              priceBuy:
                resultOrder.side === OrderSideEnum.buy ? +resultOrder.price : 0,
              idBuy:
                resultOrder.side === OrderSideEnum.buy
            ...
            this.saveProfitToDb(profitUsd, res.data?.updateTime ?? +new Date())
            this.emit('bot settings update', data)
            this.updateData({ ...data })
          }
          this.stop(true)
