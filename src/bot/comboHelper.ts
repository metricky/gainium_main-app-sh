...
class ComboBot extends createDCABotHelper<ComboBotSchema, ComboDealsSchema>() {
  ...
  private async saveMinigrid(
    ...
    if (minigrid && Object.keys(minigrid).length) {
      this.minigridDb
      ...
      (await this.generateGridsOnPrice(
        {
          ...
          _side: order.side === 'BUY' ? OrderSideEnum.buy : OrderSideEnum.sell,
          all: true,
          profitCurrency,
          orderFixedIn,
        },
  ...
  private async loadMinigrids() {
    ...
    if (!(this.serviceRestart && !this.secondRestart) || !this.minigrids.size) {
    ...
    if (deal && deal.deal.lastFilledLevel) {
      ...
      if (lowestLevel && !isNaN(lowestLevel)) {
        ...
        deal.deal.lastFilledLevel = Math.max(
          order?.dcaLevel ? order.dcaLevel - 1 : deal.deal.lastFilledLevel - 1,
          0,
        )
      }
  ...
  private async avgPrice(
    ...
    if (this.futures) {
      ...
      if (minigridId) {
        const minigrid = this.getMinigrid(minigridId)
        const minigridOrder = this.getOrderFromMap(minigrid?.schema?.dcaOrderId)
        if (minigridOrder) {
    ...
    if (minigridId) {
      ...
      if (minigrid) {
        ...
        base += this.isLong
          ...
          : minigrid.schema.initialBalances.quote / minigrid.schema.initialPrice
        quote += this.isLong
          ? minigrid.schema.initialPrice * minigrid.schema.initialBalances.base
  ...
  public async autoRebalancing(_botId: string, dealId: string) {
    ...
    if (
      (this.isLong && compareBalances.suggestedAction === BalancesAction.add) ||
      (!this.isLong &&
      ...
      this.handleLog(`Auto Rebalancing | No diff for ${dealId}. ${rawResponse}`)
    }
  }
  ...
  public async compareBalances(dealId: string) {
    ...
    try {
      ...
      const sell = orders.data.result.find((o) => o.side === OrderSideEnum.sell)
      ...
      if (this.isLong) {
        const diff = response.filledBase - response.feeBase - response.realBase
  ...
  private async findDiffCombo(
    ...
    if (settings.comboUseSmartGrids && latestPrice && !isNaN(latestPrice)) {
      ...
      if (comboSmartGridsCount && !isNaN(comboSmartGridsCount)) {
        ...
        const sells = updatedGrids.filter((g) => g.side === OrderSideEnum.sell)
  ...
  override async updateDeal(_botId: string, order: Order) {
    ...
    if (
      ...
      const gridOrders = await this.createMinigrid(findDeal.deal, order, false)
      if (!gridOrders) {
        this.handleLog(
  ...
  override async updateUsage(
    ...
    if (this.data && findDeal) {
      ...
      const hiddenQuote =
        hiddenDCA.reduce((acc, g) => acc + g.qty * g.price, 0) + boQty * boPrice
  ...
  private async updateMinigrid(order: Order) {
    ...
    if (!minigrid) {
      ...
      const findInDb = await this.minigridDb.readData({ _id: order.minigridId })
      ...
      if (findInDb.data?.result.status === ComboMinigridStatusEnum.closed) {
        this.handleLog(`Minigrid is closed, checking for deal ${order.dealId}`)
        const dealInDb = await this.dealsDb.readData({ _id: order.dealId })
    ...
    this.handleLog(`Update minigrid ${order.minigridId} ${order.clientOrderId}`)
    const { dealId } = minigrid.schema
    ...
    if (deal) {
      ...
      deal.deal.transactions = {
        ...
        sell:
          (deal.deal.transactions?.sell ?? 0) + (order.side === 'SELL' ? 1 : 0),
      }
    ...
    const wasClosed = minigrid.schema.status === ComboMinigridStatusEnum.closed
    if (grids) {
    ...
    if (minigrid.schema.status === ComboMinigridStatusEnum.closed) {
      ...
      if (deal) {
        ...
        if (minigridOrder.data?.result?.typeOrder === TypeOrderEnum.dealStart) {
          if (!deal.closeBySl) {
            deal.closeBySl = true
    ...
    if (
      ...
      deal.deal.feeBalance =
        newKucoinFee && this.isLong ? 0 : (deal.deal.feeBalance ?? 0) - feeSize
  ...
  override getInitalDealSettings(): ComboDealsSettings | undefined {
    if (this.data) {
      return this.utils.getInitalDealSettings(BotType.combo, this.data.settings)
    }
  }
  ...
  private async placeFeeOrder(_botId: string, dealId: string, orderId: string) {
    const deal = this.getDeal(dealId)
  ...
  override async startDeal(orderBo: Order) {
    ...
    if (
      ...
      let gridOrders = await this.createMinigrid(findDeal.deal, orderBo, false)
      if (!gridOrders) {
        this.handleLog(
      ...
      if (useDca) {
        ...
        for (let i = 1; i <= (ordersCount ?? 0); i++) {
          ...
          let qty = this.math.round(
            ((orderSize * (this.coinm ? ed.quoteAsset.minAmount : 1)) / price) *
              volumeVal +
            ...
            (await this.generateGridsOnPrice(
              {
                ...
                _initialPriceStart: isActiveMinigrid
                  ...
                  : price,
                _side:
                  ordersSide === 'BUY' ? OrderSideEnum.buy : OrderSideEnum.sell,
                all: true,
                profitCurrency: settings.futures
                  ? 'quote'
                  : (settings.profitCurrency ?? 'quote'),
            ...
            const qtyByGrids =
              useBase || settings.futures
                ? this.math.round(
                    ...
                    precision,
                    false,
                    (deal?.flags ?? []).includes(DCADealFlags.futuresPrecision)
                      ? !settings.futures
                      : true,
                  )
                : this.math.round(
              ...
              qty =
                useBase || settings.futures
                  ? deal?.settings.updatedComboAdjustments
                    ...
                    : qtyByGrids
                  : this.math.round(
                      (qtyByGrids / price) * (settings.futures ? 1 : feeFactor),
                      precision,
                      false,
                      true,
                    )
      ...
      if (initialOrders.length > 0) {
        ...
        if (addBase) {
          orders.push({
            ...
            side: base?.side === 'BUY' ? OrderSideEnum.buy : OrderSideEnum.sell,
        ...
        for (const m of mingrids) {
          ...
          m.initialGrids = m.initialGrids.map((io) => {
            io.price = {
              ...
              sell: this.math.round(io.price.sell, newData.priceAssetPrecision),
            }
            return io
          })
        ...
        (await this.generateGridsOnPrice(
          {
            ...
            _side:
              baseOrder.side === 'BUY' ? OrderSideEnum.buy : OrderSideEnum.sell,
            all: true,
            profitCurrency: settings.futures
              ? 'quote'
              : (settings.profitCurrency ?? 'quote'),
  ...
  override async isDealForStopLoss(d: FullDeal<ExcludeDoc<ComboDealsSchema>>) {
  ...
  private async unrealizedProfit() {
    ...
    for (const [deal, data] of this.dealsForStopLossCombo) {
      ...
      const { useSl, useTp } = await this.getAggregatedSettings(d.deal)
      const slTrigger =
        !!sl && ((this.isLong && price <= sl) || (!this.isLong && price >= sl))
      const tpTrigger =
        !!tp && ((this.isLong && price >= tp) || (!this.isLong && price <= tp))
